...
const getMutualFundRouter = (MutualFund,fundSearch) => {
    ...
    router.get('/',
        ...
        query('max-results').optional().isInt({min:10,max:100}).withMessage('Must be an integer in range [10,100].'),
        errorCheck,
        async (req, res) => {
    ...
    router.get('/:scheme_code',
        authenticator,
        param('scheme_code').isInt({min:100001,max:999999}).withMessage('Must be an integer in range [100001,999999].').custom(schemeCodeValidator),
        errorCheck,
        async (req,res) => {
            const fund = await MutualFund.findOne({'Scheme Code':req.params['scheme_code']});
